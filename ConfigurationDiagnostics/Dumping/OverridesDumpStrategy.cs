...
internal sealed class OverridesDumpStrategy : IConfigurationDumpStrategy
{
    public void Dump(IConfiguration configuration, TextWriter writer, ConfigurationDumpOptions options)
    {
        ...
        {
            WriteKeyOverrideHistory(configurationRoot, key, writer, options);
        }
    }

    private static void WriteKeyOverrideHistory(
        ...
        ConfigurationDumpOptions options)
    {
        ...
        foreach (var provider in configurationRoot.Providers)
        {
            ...
            var displayValue = SensitiveValueMasker.Mask(key, value, options);
            writer.WriteLine($"    {provider}");
            writer.WriteLine($"        {displayValue}");
        }

        var finalDisplayValue = SensitiveValueMasker.Mask(key, finalValue, options);
